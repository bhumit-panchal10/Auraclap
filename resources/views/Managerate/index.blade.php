...
@section('content')
    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">ManageRate List</h5>
                </div>
                            ...
                            <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-0">
                                ...
                                <div class="rtl:md:text-start">
                                    ...
                                    <a class="mx-1 ltr:md:text-end" title="Add New Rate"
                                        href="{{ route('manage_rate.add') }}" style="float: inline-end;">
                                        <button type="submit"
                                            class="text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                            <i class="ri-add-box-fill"></i> Add Rate
                                        </button>
                                    </a>
                                </div>
                            </div>
                              ...
                              <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                                ...
                                <form onsubmit="return validateFile()" class="tablelist-form"
                                    action="{{ route('manage_rate.index') }}" method="POST"
                            ...
                            <div class="overflow-x-auto">
                                @if (!$rates->isEmpty())
                                                ...
                                                @foreach ($rates as $rate)
                                                    <tr>
                                                        ...
                                                        <td
                                                            ...
                                                            {{ $rate->category->Category_name ?? '-' }}</td>


                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                            {{ $rate->subcategory->strSubCategoryName ?? '-' }}</td>

                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                            {{ $rate->city->cityName ?? '' }}</td>
                                                            ...
                                                            <div class="flex gap-2">
                                                                <div class="edit">
                                                                    <a class="mx-1" title="Edit"
                                                                        href="{{ route('manage_rate.edit', $rate->rate_id) }}">
                                                                        <i class="ri-edit-2-fill"></i>
                                                                    </a>

                                                                    <a class="mx-1" title="Delete" href="#"
                                                                        onclick="confirmSingleDelete({{ $rate->rate_id }})">
                                    ...
                                    <div class="flex items-center justify-between mt-5">
                                        {{ $rates->appends(request()->except('page'))->links() }}
                                    </div>
                                @else
                                    <div class="noresult">
    ...
    <script>
        function checkAll(source) {
            checkboxes = document.querySelectorAll('input[name="rate_ids[]"]');
            checkboxes.forEach(checkbox => checkbox.checked = source.checked);
        }

        function confirmBulkDelete() {
            const selected = document.querySelectorAll('input[name="rate_ids[]"]:checked');
        ...
        function confirmSingleDelete(id) {
            Swal.fire({
                ...
                if (result.value) {
                    fetch('{{ route('manage_rate.delete', ['id' => '__id__']) }}'.replace('__id__', id), {
                            method: 'DELETE',
                            headers: {
