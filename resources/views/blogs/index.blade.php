...
@section('content')
                        ...
                        <div class="card-body">
                            <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-0">
                                ...
                                <div class="rtl:md:text-start">
                                    ...
                                    @endif
                                    <a class="mx-1 ltr:md:text-end" title="Add New Blog" href="{{ route('blog.add') }}"
                                        style="float: inline-end;">
                                        <button type="submit"
                                            class="text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                            <i class="ri-add-box-fill"></i> Add New Blog
                                        </button>
                                    </a>
                                </div>
                            </div>
                            ...
                            <div class="overflow-x-auto">
                                ...
                                @if (!$Data->isEmpty())
                                    <form id="bulkDeleteForm" method="POST" action="{{ route('blog.deleteselected') }}">
                                        @csrf
                                        @method('DELETE')
                                            ...
                                            <tbody class="list form-check-all">
                                                ...
                                                @foreach ($Data as $data)
                                                    <tr class="text-center">
                                                        ...
                                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 imageURL"
                                                            style="display: flex;justify-content: center;">
                                                            @if ($data->blogImage)
                                                                <img class="img-fluid"
                                                                    src="{{ asset('/upload/Blog') . '/' . $data->blogImage }}"
                                                            ...
                                                            <div class="gap-2">
                                                                <div class="edit">
                                                                    <a class="mx-1" title="Edit"
                                                                        href="{{ route('blog.edit', $data->blogId) }}">
                                                                        <i class="ri-edit-2-fill"></i>
                                                                    </a>

                                                                    <a class="mx-1" title="Delete" href="#"
                                                                        onclick="confirmSingleDelete({{ $data->blogId }})">
    ...
    <script>
        function checkAll(source) {
            checkboxes = document.querySelectorAll('input[name="blog_ids[]"]');
            checkboxes.forEach(checkbox => checkbox.checked = source.checked);
        }

        function confirmBulkDelete() {
            const selected = document.querySelectorAll('input[name="blog_ids[]"]:checked');
        ...
        function updateStatus(status, blogId) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response == 1) {
                        $('#loading').css("display", "none");
                        $("#msgdata").html("<strong>Success !</strong> Blog Created Successfully.");
