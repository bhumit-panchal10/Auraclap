...
@section('content')
                        ...
                        <div class="card-body">
                            <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-0">
                                ...
                                <div class="rtl:md:text-start">
                                    ...
                                    @endif
                                    <a class="mx-1 ltr:md:text-end" title="Add New Career" href="{{ route('area.add') }}"
                                        style="float: inline-end;">
                                        <button type="submit"
                                            class="text-white transition-all duration-200 ease-linear btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">
                                            <i class="ri-add-box-fill"></i> Add Area
                                        </button>
                                    </a>
                                </div>
                            </div>
                            ...
                            <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                                ...
                                <form onsubmit="return validateFile()" class="tablelist-form"
                                    action="{{ route('area.index') }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">
                                        ...
                                        <div class="">
                                            ...
                                            <select name="stateId" id="stateId" onchange="getsearchcity();"
                                                class="form-select border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                                                <option selected="" value="">Select State</option>
                                                @foreach ($states as $state)
                                                    <option value="{{ $state->stateId }}"
                                                        @if (isset($StateId) && $StateId == $state->stateId) {{ 'selected' }} @endif>
                                                        {{ $state->stateName }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            ...
                                            <select name="cityId" id="cityId"
                                                ...
                                                @foreach ($cities as $city)
                                                    <option value="{{ $city->cityId }}"
                                                        @if (isset($CityId) && $CityId == $city->cityId) {{ 'selected' }} @endif>
                                                        {{ $city->cityName }}
                                                    </option>
                                                @endforeach
                                            </select>
                            ...
                            <div class="overflow-x-auto">
                                @if (!$arealist->isEmpty())
                                    <form id="bulkDeleteForm" method="POST" action="{{ route('area.deleteselected') }}">
                                        @csrf
                                        @method('DELETE')
                                                            ...
                                                            <div class="flex gap-2">
                                                                <div class="edit">
                                                                    <a class="mx-1" title="Edit"
                                                                        href="{{ route('area.edit', $area->areaId) }}">
                                    ...
                                    <div class="flex items-center justify-between mt-5">
                                        {{ $arealist->appends(request()->except('page'))->links() }}
                                    </div>
                                @else
                                    <div class="noresult">
                ...
                <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
                    ...
                    <div class="p-4">
                        <form class="tablelist-form" action="{{ route('area.update') }}" method="POST">
                            ...
                            <div class="mb-3">
                                <label for="email-field" class="">Pickup Start Time <span
                            ...
                            <div class="mb-3">
                                <label for="email-field" class="">Pickup End Time <span
                                        class="text-red-500"></span></label>
    ...
    <script>
        function getsearchcity() {
            ...
            var url = "{{ route('area.state_city_mapping') }}";
