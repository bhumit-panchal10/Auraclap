    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Pending Order List</h5>
                </div>
                            ...
                            <div class="overflow-x-auto">
                                @if (!$pendingorderlist->isEmpty())
                                                ...
                                                @foreach ($pendingorderlist as $pendingorder)
                                                    <tr>
                                                        {{-- <th class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500"
                                                            ...
                                                            <input
                                                                class="border rounded-sm appearance-none cursor-pointer size-4 bg-slate-100 border-slate-200 dark:bg-zink-600 dark:border-zink-500 checked:bg-custom-500 checked:border-custom-500 dark:checked:bg-custom-500 dark:checked:border-custom-500 checked:disabled:bg-custom-400 checked:disabled:border-custom-400"
                                                                type="checkbox" name="order_ids[]"
                                                                value="{{ $pendingorder->iOrderId }}">
                                                        </th> --}}

                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 id">
                                                            {{ $i + $pendingorderlist->perPage() * ($pendingorderlist->currentPage() - 1) }}
                                                        </td>
                                                        ...
                                                        <td
                                                            ...
                                                            {{ $pendingorder->Customer_name ?? '-' }}</td>


                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                            {{ $pendingorder->Customer_phone ?? '-' }}</td>

                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                            {{ $pendingorder->Pincode ?? '-' }}</td>


                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                            {{ $pendingorder->Customer_Address ?? '-' }}</td>
                                                        ...
                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                            {{ date('d-m-Y', strtotime($pendingorder->order_date)) ?? '-' }}
                                                        </td>

                                                        <td
                                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                            {{ $pendingorder->payment_mode == 1 ? 'online' : 'cash' }}
                                                        </td>
                                    ...
                                    <div class="flex items-center justify-between mt-5">
                                        {{ $pendingorderlist->appends(request()->except('page'))->links() }}
                                    </div>
                                @else
                                    <div class="noresult">
