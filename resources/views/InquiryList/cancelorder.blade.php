    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Cancel Order List</h5>
                </div>
                            ...
                            <div class="overflow-x-auto">
                                @if (!$cancelorderlist->isEmpty())
                                        ...
                                        <table class="w-full whitespace-nowrap" id="customerTable">
                                            <thead class="bg-slate-100 dark:bg-zink-600">
                                                <tr>
                                                    ...
                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 ltr:text-left rtl:text-right"
                                                        data-sort="state_name">Order Date</th>

                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 ltr:text-left rtl:text-right"
                                                        data-sort="state_name">Payment Mode</th>

                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 ltr:text-left rtl:text-right"
                                                        data-sort="state_name">Order ID</th>

                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 ltr:text-left rtl:text-right"
                                                        data-sort="state_name">Refund</th>
                                                    ...
                                                    @foreach ($cancelorder->orders as $order)
                                                        <tr>
                                                            <td
                                                                ...
                                                                {{ $i + $cancelorderlist->perPage() * ($cancelorderlist->currentPage() - 1) }}
                                                            </td>
                                                            ...
                                                            <td
                                                                ...
                                                                {{ $cancelorder->Customer_name ?? '-' }}</td>

                                                            <td
                                                                class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                                {{ $cancelorder->Customer_phone ?? '-' }}</td>

                                                            <td
                                                                class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                                {{ $cancelorder->Pincode ?? '-' }}</td>


                                                            <td
                                                                class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                                {{ \Illuminate\Support\Str::limit($cancelorder->Customer_Address, 40, '...') }}

                                                            <td
                                                                class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                                {{ date('d-m-Y', strtotime($cancelorder->order_date)) ?? '-' }}
                                                            </td>

                                                            <td
                                                                class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                                {{ $cancelorder->payment_mode == 1 ? 'online' : 'cash' }}
                                                            </td>
                                                            ...
                                                            <td
                                                                ...
                                                                @if (($cancelorder->isRefund ?? 0) == 1)
                                                                    Yes
                                                                @else
                                                                    <a href="{{ route('RefundPaymentflag', $cancelorder->iOrderId) }}"
                                    ...
                                    <div class="flex items-center justify-between mt-5">
                                        {{ $cancelorderlist->appends(request()->except('page'))->links() }}
                                    </div>
                                @else
                                    <div class="noresult">
