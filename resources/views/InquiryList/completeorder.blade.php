...
@section('title', 'CompleteOrder List')
@section('content')
    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">CompleteOrder List</h5>
                </div>
                            ...
                            <div class="overflow-x-auto">
                                @if (!$completeorderlist->isEmpty())
                                        ...
                                        <table class="w-full whitespace-nowrap" id="customerTable">
                                            <thead class="bg-slate-100 dark:bg-zink-600">
                                                <tr>
                                                    ...
                                                    <th class="sort px-3.5 py-2.5 font-semibold border-b border-slate-200 dark:border-zink-500 ltr:text-left rtl:text-right"
                                                        data-sort="state_name">Complete DateTime</th>
                                            ...
                                            <tbody class="list form-check-all">
                                                ...
                                                @foreach ($completeorderlist as $completeorder)
                                                    @foreach ($completeorder->orders as $order)
                                                        <tr>
                                                            <td
                                                                class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 id">
                                                                {{ $i + $completeorderlist->perPage() * ($completeorderlist->currentPage() - 1) }}
                                                            </td>
                                                            ...
                                                            <td
                                                                ...
                                                                {{ $completeorder->Customer_name ?? '-' }}</td>


                                                            <td
                                                                class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                                {{ $completeorder->Customer_phone ?? '-' }}</td>

                                                            <td
                                                                class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                                {{ $completeorder->Pincode ?? '-' }}</td>


                                                            <td
                                                                class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                                {{ \Illuminate\Support\Str::limit($completeorder->Customer_Address, 40, '...') }}

                                                            <td
                                                                class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                                {{ date('d-m-Y H:i:s', strtotime($completeorder->created_at)) ?? '-' }}
                                                            </td>
                                                            ...
                                                            <td
                                                                class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 customer_name">
                                                                {{ $completeorder->payment_mode == 1 ? 'online' : 'cash' }}
                                                            </td>
                                    ...
                                    <div class="flex items-center justify-between mt-5">
                                        {{ $completeorderlist->appends(request()->except('page'))->links() }}
                                    </div>
                                @else
                                    <div class="noresult">
